...
struct
  ...
  fun keybdLines file prompt1 prompt2 ducer =
      let val pr = ref prompt1
          ...
          fun put s =
              case (pr := prompt1; S.front s)
                of S.Cons (x, s) => S.Cons (x, S.lazy (fn () => put s))
                 | S.Nil => S.Nil
      in
      ...
      let fun get file =
              case T.inputLine file
                of NONE => (T.closeIn file; S.Nil)
                 | SOME(ln) => S.Cons (ln, S.lazy (fn() => get file))
      in
  ...
  fun line (nil, s) = S.front s
    | line (l as (h::t), s) = S.Cons ((h, l), S.lazy (fn () => line (t, s)))

  fun linesToChars s =
      let fun get s =
              case S.front s
                of S.Cons (x, s) => line (explode x, S.lazy (fn () => get s))
                 | S.Nil => S.Nil
      in
        S.lazy (fn () => get s)
      end
